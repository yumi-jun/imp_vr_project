...
{
  public class QwertyKeyboardController : AbstractKeboardController
  {

    [SerializeField] private List<GroupController> _groupControllers;
    [SerializeField] private SpriteRenderer _circle;
    [SerializeField] protected NodeskManager _inputSystemController;
    ...
    private void TouchMovePhase1(Vector2 position)
    {
      ...
      if (position.magnitude >= 0.99)
      {
        Phase1EndAnimation(index, position.magnitude, position);
        _inputSystemController.ChangeVibrate();
    ...
    private void TouchMovePhase2(Vector2 position)
    {
      ...
      var index = select1(angle, position.magnitude, _selectedtGroupController.Index);
      
      if (index != -1 && position.magnitude > 0.99)
      {
        _lastSelectIndex = index;
        Debug.Log(_lastSelectIndex);
      }
    ...
    {
      _inputSystemController.SubmitVibrate();
      _circle.transform.localPosition = Vector3.zero;
      
      var index = _lastSelectIndex;
      Debug.Log(_lastSelectIndex);
      string key = _selectedtGroupController.GetKey(index);
      _inputSystemController.FireEvent(new NodeskTypeEventInfo()
      {
